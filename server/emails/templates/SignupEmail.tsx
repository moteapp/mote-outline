 ...
 */
export default class SignupEmail extends BaseEmail<Props, Record<string, any>> {
  protected subject() {
    return "Magic signup link";
  }

  protected preview(): string {
    return `Here’s your link to signup to ${env.APP_NAME}.`;
  }

  protected renderAsText({ token, teamUrl, client }: Props): string {
    return `
Use the link below to signup to ${env.APP_NAME}:
...
`;
  ...
  protected render({ token, client, teamUrl }: Props) {
    if (env.isDevelopment) {
      logger.debug("email", `Sign-Up link: ${this.signinLink(token, client)}`);
    }
    ...
    return (
      ...
      >
        ...
        <Body>
          <Heading>Magic Sign-up Link</Heading>
          ...
          <p>
            ...
            team’s sign-up page at: <a href={teamUrl}>{teamUrl}</a>
          </p>
        </Body>
