...
router.post(
    ...
    async (ctx: APIContext<T.DocumentsUpdateReq>) => {
        ...
        const { id, insightsEnabled, publish, collectionId, ...input } =
            ctx.input.body;
        const editorVersion = ctx.headers['x-editor-version'] as
            | string
            | undefined;
        ...
        if (parentDocumentId) {
            ...
            if (!parent.publishedAt) {
                throw InvalidRequestError(
                    'Cannot duplicate document inside a draft'
                );
            }
        }
...
router.post(
    ...
    async (ctx: APIContext<T.DocumentsMoveReq>) => {
        ...
        if (parentDocumentId) {
            ...
            if (!parent.publishedAt) {
                throw InvalidRequestError(
                    'Cannot move document inside a draft'
                );
            }
        }
