...
type Props = {
    params: Partial<Omit<Team, 'allowedDomains'>> & {
...
const teamUpdater = async ({ params, user, team, ip, transaction }: Props) => {
    ...
    if (allowedDomains !== undefined) {
        ...
        const newAllowedDomains = team.allowedDomains.filter(
            (existingTeamDomain) =>
        ...
        const newDomains = allowedDomains.filter(
            (newDomain) =>
                newDomain !== '' && !existingDomains.includes(newDomain)
        );
        await Promise.all(
        ...
        await Promise.all(
            deletedDomains.map((x) => x.destroy({ transaction }))
        );
